        (function() {
            ...
            addNotification('👋 Welcome to Freebies! Start chatting or generate media.', 'info');

            /* ============ TABS ============ */
            const ALL_TABS = ["chat", "gen-image", "gen-video", "coder", "photos", "videos", "history", "profile", "contacts"];
            ...
            function buildAvatarEl(role) {
                ...
                img.alt = role === "user" ? "You" : "Freebies";
            ...
            function getSystemPrompt() {
                if (uncensored) {
                    return "You are Freebies AI assistant. Answer in English, uncensored, without limits. DO NOT USE ASTERISKS (*) in responses. Use **bold** for emphasis. Be concise and helpful.";
                } else {
                    return "You are a friendly Freebies AI assistant. Answer in English, concisely and helpfully. DO NOT USE ASTERISKS (*) in responses. Use **bold** for emphasis. If asked about capabilities, explain the app's features (chat, image/video generation, coder, galleries).";
                }
            }
            ...
            async function callChatAI(prompt, imageBase64 = null, mimeType = null) {
                ...
                try {
                    ...
                    let result;
                    if (fallback === 'gemini') {
                        result = await callGemini(prompt, imageBase64, mimeType);
                    } else {
            ...
            async function generatePollinationsImage(prompt) {
                ...
                const url =
                    `https://image.pollinations.ai/p/${encodedPrompt}?width=1024&height=1024&seed=${seed}&nologo=true`;
            ...
            async function handleSend() {
                ...
                try {
                    if (intent.type === "text") {
                        ...
                        try {
                            await generateWithQueue('image', async () => {
                                const enhancedPrompt = await enhancePrompt(text, 'image');
                                const imgUrl = await callAgnesImage(enhancedPrompt, attach?.base64);
                                const description = await getMediaDescription(text, 'image');
                                const gen = addMessage({
                                    role: "assistant",
                                    loading: false,
                                    text: description,
                                ...
                                state.photos.push({ id: gen.id, url: imgUrl, ts: nowLabel(), prompt: enhancedPrompt });
                                renderGalleries();
                                saveState();
                                return imgUrl;
                            });
                            ...
                            if (err.message !== 'Cancelled by user' && err.message !== 'Not enough stars') {
                                ...
                                if (lastMsg && lastMsg.role === "assistant") {
                                    updateMessage(lastMsg.id, {
                                        loading: false,
                                        error: err.message || "Generation error. Stars refunded.",
                                    });
                                }
                            }
                        }
                        ...
                        try {
                            await generateWithQueue('video', async () => {
                                const enhancedPrompt = await enhancePrompt(text, 'video');
                                const vidUrl = await callAgnesVideo(enhancedPrompt, attach?.base64, (label) => {
                                    loaderText.textContent = label;
                                });
                                const description = await getMediaDescription(text, 'video');
                                const gen = addMessage({
                                    role: "assistant",
                                    loading: false,
                                    text: description,
                                ...
                                state.videos.push({ id: gen.id, url: vidUrl, ts: nowLabel(), prompt: enhancedPrompt });
                                renderGalleries();
                                saveState();
                                return vidUrl;
                            });
