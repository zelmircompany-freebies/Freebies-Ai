        ...
        // then safely opens the sign-in modal. Used by every "Sign In" entry point so
        // a slow/blocked network never results in a silently-dead button.
        // Returns true if it managed to open sign-in, false otherwise (so callers
        // can show their own inline error instead of relying on a bare alert()).
        if (!window.openClerkSignInSafe) {
            window.openClerkSignInSafe = async function (btnEl, opts) {
                ...
                if (window.Clerk && clerkReady) {
                    try {
                        ...
                        console.error('❌ Clerk.openSignIn() threw:', e);
            ...
            if (user) {
                ...
                if (accountInfoBtn) {
                    ...
                    accountInfoBtn.onclick = () => {
                        ...
                        else if (window.openClerkSignInSafe) window.openClerkSignInSafe(accountInfoBtn);
                    };
                }
                if (logoutBtn) {
                ...
                settingsButtons.forEach(btn => {
                    if (btn) {
                        btn.onclick = () => {
                            if (isRu) { const gate = document.getElementById('authGate'); if (gate) gate.classList.remove('hidden'); }
                            else if (window.openClerkSignInSafe) window.openClerkSignInSafe(btn);
                        };
                    }
                });
