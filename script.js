        (function() {
            ...
            const PromoEngine = {
                // First 7 days after signup: discounted video price (3⭐ instead of 10⭐) + subscription discounts.
                ...
                // Discounts on first-time subscription purchase, valid the first 7 days.
                subscriptionDiscount: () => {
                    if (!PromoEngine.isFirstWeek()) return { pro: 0, diamond: 0 };
                    return { pro: 70, diamond: 50 };
                },
            };
            ...
            // 4) subscription plan discount (Pro: 5⭐, Diamond: 3⭐)
            // 5) otherwise the standard price
            function getVideoPrice() {
                if (!state.firstVideoGenUsed) return PRICE_VIDEO_FIRST;
                if (PromoEngine.isFirstWeek()) return 3;
                const plan = (window.PlanSystem && window.PlanSystem.getCurrentPlan) ? window.PlanSystem.getCurrentPlan() : 'free';
            ...
            function openDonateModal(starsCount, priceAmount) {
                ...
                const plan = (window.PlanSystem && window.PlanSystem.getCurrentPlan) ? window.PlanSystem.getCurrentPlan() : 'free';
                const discountPct = plan === 'diamond' ? 70 : (plan === 'pro' ? 35 : 0);
                if (discountPct > 0) {
                    const discPrice = Math.round(priceAmount * (1 - discountPct / 100));
                    donateTariffInfo.innerHTML = `${starsCount} stars for <s style="opacity:.6">${priceAmount} ₽</s> <b>${discPrice} ₽</b> (−${discountPct}% ${plan === 'diamond' ? 'Diamond' : 'Pro'})`;
                } else {
            ...
            /* ============ PLAN SYSTEM (Free / Pro / Diamond) ============
               Subscriptions are paid manually via DonationAlerts (same flow as star packs) and
               activated by the team. Until there's a server-side field for this, the active plan
               is stored locally per-browser so the UI (prices, unlocked tabs, profile badge) can
               react correctly. An admin/support flow can call window.PlanSystem.setPlan('pro'|'diamond'|'free')
               from the console after confirming a payment, or this can be wired up to a real
               subscriptions table later. */
            const PLAN_STORAGE_KEY = 'freebies_user_plan';
            function getCurrentPlan() {
                try {
                    const p = localStorage.getItem(PLAN_STORAGE_KEY);
                    if (p === 'pro' || p === 'diamond') return p;
                } catch (e) {}
                return 'free';
            }
            function setPlan(plan) {
                try { localStorage.setItem(PLAN_STORAGE_KEY, plan); } catch (e) {}
                refreshPlanUI();
            }
            const PlanSystem = { getCurrentPlan, setPlan };
            window.PlanSystem = PlanSystem;

            function refreshPlanUI() {
                const plan = getCurrentPlan();
                // Profile plan label + badge icon
                const profilePlanEl = document.getElementById('profilePlan');
                if (profilePlanEl && window.AppAuth && window.AppAuth.getProvider()) {
                    const labels = { free: 'Free', pro: 'Pro ⭐', diamond: 'Diamond 💎' };
                    profilePlanEl.textContent = labels[plan] || 'Free';
                }
                // Unlimited Generation tab: locked for Free & Pro, unlocked for Diamond (item 8)
                ...
                ugTabBtns.forEach(btn => {
                    ...
                    if (plan === 'diamond') {
                        ...
                        btn.classList.add('ug-locked');
                        if (badge) { badge.style.display = ''; badge.textContent = 'Pro+'; }
                    }
                });
            ...
            // Block access to Unlimited Generation for Free/Pro users (item 8)
            document.addEventListener('click', (e) => {
                const btn = e.target.closest('[data-tab="unlimited-gen"]');
                if (!btn) return;
                if (getCurrentPlan() !== 'diamond') {
                    e.stopImmediatePropagation();
                    e.preventDefault();
                    alert('♾️ Unlimited Generation is available for Diamond subscribers only. Upgrade in the Star Shop → Subscribe tab.');
                    setTab('star-shop');
                    setTimeout(() => { const t = document.querySelector('[data-shop-tab="subscribe"]'); if (t) t.click(); }, 50);
                }
            }, true);

            /* ---- Star Shop: Stars / Subscribe tab switcher ---- */
            const starShopTabs = document.getElementById('starShopTabs');
            const starShopPanelStars = document.getElementById('starShopPanelStars');
            const starShopPanelSubscribe = document.getElementById('starShopPanelSubscribe');
            if (starShopTabs) {
                starShopTabs.querySelectorAll('.star-shop-tab-btn').forEach(btn => {
                    btn.addEventListener('click', () => {
                        starShopTabs.querySelectorAll('.star-shop-tab-btn').forEach(b => b.classList.remove('selected'));
                        btn.classList.add('selected');
                        const tab = btn.dataset.shopTab;
                        if (starShopPanelStars) starShopPanelStars.classList.toggle('hidden', tab !== 'stars');
                        if (starShopPanelSubscribe) starShopPanelSubscribe.classList.toggle('hidden', tab !== 'subscribe');
                    });
                });
            }

            /* ---- Subscribe panel: first-week discount + Star pack discount note ---- */
            function renderSubscribePanel() {
                const banner = document.getElementById('subFirstWeekBanner');
                const discount = PromoEngine.subscriptionDiscount();
                const inFirstWeek = discount.pro > 0;
                if (banner) banner.classList.toggle('hidden', !inFirstWeek);

                const lang = (() => { try { return localStorage.getItem('freebies_lang') || 'en'; } catch (e) { return 'en'; } })();

                function applySubPrice(planKey, discountPct, oldElId, newElId) {
                    const card = document.querySelector(`.sub-plan[data-plan="${planKey}"]`);
                    if (!card) return;
                    const rub = parseInt(card.dataset.priceRub, 10);
                    const usd = parseInt(card.dataset.priceUsd, 10);
                    const oldEl = document.getElementById(oldElId);
                    const newEl = document.getElementById(newElId);
                    if (!oldEl || !newEl) return;
                    if (discountPct > 0) {
                        const discRub = Math.round(rub * (1 - discountPct / 100));
                        const discUsd = Math.round(usd * (1 - discountPct / 100));
                        oldEl.textContent = lang === 'ru' ? `${rub} ₽` : `$${usd}`;
                        oldEl.classList.remove('hidden');
                        newEl.textContent = lang === 'ru' ? `${discRub} ₽ / 3 мес (−${discountPct}%)` : `$${discUsd} / 3 mo (−${discountPct}%)`;
                    } else {
                        oldEl.classList.add('hidden');
                        newEl.textContent = lang === 'ru' ? `${rub} ₽ / 3 мес` : `$${usd} / 3 mo`;
                    }
                }
                applySubPrice('pro', discount.pro, 'proPriceOld', 'proPriceNew');
                applySubPrice('diamond', discount.diamond, 'diamondPriceOld', 'diamondPriceNew');

                // Star-pack purchase discount note for Pro (-35%) / Diamond (-70%) subscribers
                const plan = getCurrentPlan();
                const note = document.getElementById('starPackDiscountNote');
                if (note) {
                    if (plan === 'pro') note.textContent = lang === 'ru' ? '💎 У вас скидка −35% на покупку звёзд как у Pro-подписчика.' : '💎 You get −35% off star purchases as a Pro subscriber.';
                    else if (plan === 'diamond') note.textContent = lang === 'ru' ? '💎 У вас скидка −70% на покупку звёзд как у Diamond-подписчика.' : '💎 You get −70% off star purchases as a Diamond subscriber.';
                    else note.textContent = '';
                }

                // Reflect current plan on the Free card's button
                document.querySelectorAll('.sub-plan').forEach(card => {
                    const cardPlan = card.dataset.plan;
                    const btn = card.querySelector('.plan-buy-btn');
                    if (!btn) return;
                    if (cardPlan === plan) {
                        btn.disabled = true;
                        btn.textContent = lang === 'ru' ? 'Текущий план' : 'Current Plan';
                        btn.classList.add('plan-buy-outline');
                    } else if (cardPlan !== 'free') {
                        btn.disabled = false;
                        btn.textContent = lang === 'ru' ? 'Оформить' : 'Subscribe';
                    }
                });
            }

            /* ---- Subscription payment modal (DonationAlerts flow, manual activation) ---- */
            const subPaymentModal = document.getElementById('subPaymentModal');
            const subPaymentTitle = document.getElementById('subPaymentTitle');
            const subPaymentTariff = document.getElementById('subPaymentTariff');
            const subPaymentAuthWarning = document.getElementById('subPaymentAuthWarning');
            const subPaymentIdBlock = document.getElementById('subPaymentIdBlock');
            const subPaymentUserId = document.getElementById('subPaymentUserId');
            const subPaymentCopyBtn = document.getElementById('subPaymentCopyBtn');
            const subPaymentCloseBtn = document.getElementById('subPaymentCloseBtn');
            const subPaymentPayLink = document.getElementById('subPaymentPayLink');

            function openSubPaymentModal(planKey) {
                const card = document.querySelector(`.sub-plan[data-plan="${planKey}"]`);
                if (!card || !subPaymentModal) return;
                const discount = PromoEngine.subscriptionDiscount();
                const discountPct = planKey === 'pro' ? discount.pro : discount.diamond;
                const rub = parseInt(card.dataset.priceRub, 10);
                const finalRub = discountPct > 0 ? Math.round(rub * (1 - discountPct / 100)) : rub;

                if (subPaymentTitle) subPaymentTitle.textContent = planKey === 'pro' ? '💎 Subscribe to Pro' : '💎 Subscribe to Diamond';
                if (subPaymentTariff) subPaymentTariff.textContent = `${finalRub} ₽ for 3 months (${planKey === 'pro' ? 'Pro' : 'Diamond'})` + (discountPct > 0 ? ` — first-week −${discountPct}% applied` : '');

                const userId = (window.AppAuth ? window.AppAuth.getUserId() : null) || null;
                if (!userId) {
                    if (subPaymentAuthWarning) subPaymentAuthWarning.classList.remove('hidden');
                    if (subPaymentIdBlock) subPaymentIdBlock.classList.add('hidden');
                } else {
                    if (subPaymentAuthWarning) subPaymentAuthWarning.classList.add('hidden');
                    if (subPaymentIdBlock) subPaymentIdBlock.classList.remove('hidden');
                    if (subPaymentUserId) subPaymentUserId.textContent = userId;
                    if (subPaymentCopyBtn) subPaymentCopyBtn.textContent = 'Copy my ID';
                }
                subPaymentModal.classList.add('active');
            }
            function closeSubPaymentModal() {
                if (subPaymentModal) subPaymentModal.classList.remove('active');
            }
            document.querySelectorAll('.sub-buy-btn').forEach(btn => {
                btn.addEventListener('click', () => openSubPaymentModal(btn.dataset.plan));
            });
            if (subPaymentCopyBtn) {
                subPaymentCopyBtn.addEventListener('click', () => {
                    const id = window.AppAuth ? window.AppAuth.getUserId() : null;
                    if (!id) return;
                    navigator.clipboard.writeText(id).then(() => {
                        subPaymentCopyBtn.textContent = 'Copied!';
                        setTimeout(() => { subPaymentCopyBtn.textContent = 'Copy my ID'; }, 1800);
                    }).catch(() => { alert('Could not copy ID. Copy manually: ' + id); });
                });
            }
            if (subPaymentCloseBtn) subPaymentCloseBtn.addEventListener('click', closeSubPaymentModal);
            if (subPaymentModal) {
                subPaymentModal.addEventListener('click', (e) => { if (e.target === subPaymentModal) closeSubPaymentModal(); });
            }
            if (subPaymentPayLink) {
                subPaymentPayLink.addEventListener('click', () => {
                    showToast('⏱ Your subscription will be activated within a day after payment.<br>If not, email <b>zelmir.company@gmail.com</b>', 7000);
                });
            }
            ...
            function buildWelcomeText(lang, firstName) {
                if (lang === 'ru') {
                    return {
                        ...
                        body: `Мы заметили, что ты зарегистрировался в Freebies Ai — твоем проводнике в мир бесплатного творчества! 🎨<br><br>` +
                              ...
                              `Первые 7 дней генерация видео будет стоить всего 3 звезды вместо 10! Скидка на покупку подписки! Это наш подарок специально для тебя ❤️<br><br>` +
                ...
                return {
                    ...
                    body: `We noticed you just signed up for Freebies AI — your gateway to free creativity! 🎨<br><br>` +
                          ...
                          `🎉 First-week bonus:<br>` +
                          `For the first 7 days, video generation costs just 3 stars instead of 10! Plus a discount on subscriptions! This is our gift, just for you ❤️<br><br>` +
                ...
                try {
                    if (provider === 'supabase') {
                        ...
                        if (sbUser) {
                            ...
                            await window.AppAuth.supabase.saveProfile({ name: newName });
                        }
                    }
            ...
            const I18N = {
                en: {
                    ...
                    // Star shop tabs & plans
                    star_shop_title: "Star Shop",
                    star_shop_sub: "Buy stars or subscribe for the best value",
                    tab_stars: "⭐ Stars",
                    tab_subscribe: "💎 Subscribe",
                    sub_first_week_banner: "First 7 days after signup: −70% on Pro, −50% on Diamond!",
                    plan_free_tag: "Always Free",
                    plan_free_f1: "✓ 1000 ⭐ every 3 months",
                    plan_free_f2: "✓ Unlimited AI Chat",
                    plan_free_f3: "✓ Free image generation (0 ⭐)",
                    plan_free_f4: "✓ Video generation for 10 ⭐",
                    plan_current_btn: "Current Plan",
                    plan_pro_tag: "Pro",
                    plan_pro_f1: "✓ 5000 ⭐ every 3 months",
                    plan_pro_f2: "✓ Unlimited AI Chat",
                    plan_pro_f3: "✓ Free image generation (0 ⭐)",
                    plan_pro_f4: "✓ Video generation for 5 ⭐ (instead of 10)",
                    plan_pro_f5: "✓ −35% on star purchases",
                    plan_pro_f6: "✓ Unique profile icon",
                    plan_diamond_tag: "Diamond",
                    plan_diamond_f1: "✓ 10000 ⭐ every 3 months",
                    plan_diamond_f2: "✓ Unlimited AI Chat",
                    plan_diamond_f3: "✓ Free image generation (0 ⭐)",
                    plan_diamond_f4: "✓ Video generation for 3 ⭐ (instead of 10)",
                    plan_diamond_f5: "✓ −70% on star purchases",
                    plan_diamond_f6: "✓ Unlimited Generation tab unlocked",
                    plan_diamond_f7: "✓ Unique profile icon",
                    plan_subscribe_btn: "Subscribe",
                    sub_footnote: "🔒 Subscriptions are activated manually after payment confirmation — see the payment window for details.",
                    sub_modal_subtitle: "Pay via DonationAlerts, subscription is activated manually within a day",
                    sub_id_label: "Your User ID (include in payment message)",
                    sub_modal_eta: "Your subscription will be activated within a day after payment. If not,",
                    sub_modal_go_pay: "Go to payment",
                    close_btn: "Close",
                ...
                ru: {
                    ...
                    star_shop_sub: "Покупайте звёзды или оформите подписку — так выгоднее",
                    tab_stars: "⭐ Звёзды",
                    tab_subscribe: "💎 Подписка",
                    sub_first_week_banner: "Первые 7 дней после регистрации: −70% на Pro, −50% на Diamond!",
                    plan_free_tag: "Всегда бесплатно",
                    plan_free_f1: "✓ 1000 ⭐ каждые 3 месяца",
                    plan_free_f2: "✓ Безлимит на Chat с искусственным интеллектом",
                    plan_free_f3: "✓ Бесплатная генерация картинок (0 ⭐)",
                    plan_free_f4: "✓ Генерация видео за 10 ⭐",
                    plan_current_btn: "Текущий план",
                    plan_pro_tag: "Pro",
                    plan_pro_f1: "✓ 5000 ⭐ каждые 3 месяца",
                    plan_pro_f2: "✓ Безлимит на Chat с искусственным интеллектом",
                    plan_pro_f3: "✓ Бесплатная генерация картинок (0 ⭐)",
                    plan_pro_f4: "✓ Генерация видео за 5 ⭐ (вместо 10)",
                    plan_pro_f5: "✓ Скидка −35% при покупке звёзд",
                    plan_pro_f6: "✓ Уникальная иконка в профиле",
                    plan_diamond_tag: "Diamond",
                    plan_diamond_f1: "✓ 10000 ⭐ каждые 3 месяца",
                    plan_diamond_f2: "✓ Безлимит на Chat с искусственным интеллектом",
                    plan_diamond_f3: "✓ Бесплатная генерация картинок (0 ⭐)",
                    plan_diamond_f4: "✓ Генерация видео за 3 ⭐ (вместо 10)",
                    plan_diamond_f5: "✓ Скидка −70% при покупке звёзд",
                    plan_diamond_f6: "✓ Открыта вкладка Unlimited Generation",
                    plan_diamond_f7: "✓ Уникальная иконка в профиле",
                    plan_subscribe_btn: "Оформить",
                    sub_footnote: "🔒 Подписки активируются вручную после подтверждения оплаты — подробности в окне оплаты.",
                    sub_modal_subtitle: "Оплата через DonationAlerts, подписка активируется вручную в течение дня",
                    sub_id_label: "Ваш ID пользователя (укажите в сообщении при оплате)",
                    sub_modal_eta: "Ваша подписка будет активирована в течение дня после оплаты. Если нет,",
                    sub_modal_go_pay: "Перейти к оплате",
                    close_btn: "Закрыть",
            ...
            function applyLanguage(lang) {
                ...
                if (typeof renderSubscribePanel === 'function') renderSubscribePanel();
            }
            let savedLang = "en";
