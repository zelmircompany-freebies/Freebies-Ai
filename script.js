        (function() {
            ...
            const imgModelGroup = $("#imgModelGroup");
            const imgModeModeBtnImage = $('#imgModeGroup .mode-btn[data-mode="image"]');

            function setImgModel(model) {
                imgModel = model;
                $$("#imgModelGroup .model-card").forEach(c => c.classList.toggle("selected", c.dataset.model === model));
            ...
            if (imgModelGroup) {
                $$("#imgModelGroup .model-card").forEach(card => {
                    card.addEventListener("click", () => setImgModel(card.dataset.model));
                });
            }
            ...
            const vidModelGroup = $("#vidModelGroup");
            const vidDuration10Btn = $('#vidDurationGroup .duration-btn[data-duration="10"]');

            function setVidModel(model) {
                vidModel = model;
                $$("#vidModelGroup .model-card").forEach(c => c.classList.toggle("selected", c.dataset.model === model));
            ...
            if (vidModelGroup) {
                $$("#vidModelGroup .model-card").forEach(card => {
                    card.addEventListener("click", () => setVidModel(card.dataset.model));
                });
            }
