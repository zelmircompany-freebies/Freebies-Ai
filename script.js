        (function() {
            ...
            /* ============ AUTH HELPERS (provider-agnostic: Clerk or Supabase) ============ */
            function isAuthed() {
            ...
            function openLoginPrompt() {
                ...
                } else if (window.openClerkSignInSafe) {
                    window.openClerkSignInSafe();
            ...
            // Supabase users (Russia): profile & stars are synced server-side
            ...
            async function persistStarsToClerk() {
                // Supabase users (Russia): persist server-side.
                const sbUser = window.AppAuth?.supabase?.getUser?.();
                if (sbUser) {
                    await window.AppAuth.supabase.saveProfile({ stars: state.stars });
                    return;
        ...
        // The Clerk <script> tag loads with `async`, so it may not have finished
        // executing yet by the time `load` fires (this gets worse on VPN / slow
        // networks). Poll for it instead of giving up on the first check.
        function waitForClerkScript(timeoutMs = 10000) {
            return new Promise((resolve) => {
                const start = Date.now();
                (function poll() {
                    if (typeof window.Clerk !== 'undefined') { resolve(true); return; }
        ...
        // Waits for Clerk to be fully ready (script loaded AND Clerk.load() resolved),
        // then safely opens the sign-in or sign-up modal. Used by every auth entry
        // point so a slow/blocked network never results in a silently-dead button.
        // mode: 'signIn' (default) or 'signUp'.
        // Returns true if it managed to open the modal, false otherwise (so callers
        // can show their own inline error instead of relying on a bare alert()).
        if (!window.openClerkAuthSafe) {
            window.openClerkAuthSafe = async function (btnEl, mode, opts) {
                mode = mode || 'signIn';
                const silent = opts && opts.silent;
                const originalText = btnEl ? btnEl.textContent : '';
                if (btnEl) { btnEl.disabled = true; btnEl.textContent = 'Loading…'; }
                const start = Date.now();
                while (!clerkReady && Date.now() - start < 10000) {
                    await new Promise((r) => setTimeout(r, 150));
                }
                if (btnEl) { btnEl.disabled = false; btnEl.textContent = originalText; }
                if (window.Clerk && clerkReady) {
                    try {
                        if (mode === 'signUp') window.Clerk.openSignUp();
                        else window.Clerk.openSignIn();
                        return true;
                    } catch (e) {
                        console.error(`❌ Clerk.open${mode === 'signUp' ? 'SignUp' : 'SignIn'}() threw:`, e);
                        if (!silent) alert('Sign-in failed to open: ' + (e?.message || 'unknown error'));
                        return false;
                    }
                } else {
                    console.error('❌ Clerk is not ready. window.Clerk:', typeof window.Clerk, 'clerkReady:', clerkReady);
                    if (!silent) alert('Could not connect to the sign-in service. Please check your connection (VPN/ad-block can sometimes block this) and try again.');
                    return false;
                }
            };
        }
        if (!window.openClerkSignInSafe) {
            window.openClerkSignInSafe = function (btnEl, opts) {
                return window.openClerkAuthSafe(btnEl, 'signIn', opts);
            };
        }

        window.addEventListener('load', async () => {
            const scriptLoaded = await waitForClerkScript();
            if (!scriptLoaded) {
        ...
        function updateUI() {
            ...
            // or Supabase, via window.AppAuth.
            const isRu = window.AppAuth ? window.AppAuth.isRussia() : false;
            const provider = window.AppAuth ? window.AppAuth.getProvider() : null;
            const user = provider === 'clerk' ? window.Clerk?.user : (provider === 'supabase' ? window.AppAuth.supabase.getUser() : null);

            // Note: we no longer auto-open Clerk's sign-in modal here — the
            // authGate overlay (Supabase + Clerk) already handles first-visit
            // sign-in/sign-up for everyone, so a second modal on top of it
            ...
            if (user) {
                ...
                if (accountInfoBtn) {
                    accountInfoBtn.textContent = provider === 'supabase' ? `👤 ${fullName}` : `👤 ${email}`;
                ...
                if (userId !== lastSeenUserId) {
                    ...
                    } else if (provider === 'supabase' && typeof window.syncStarsFromSupabase === 'function') {
                ...
                if (lastSeenUserId !== null) {
                    ...
                    if (provider === 'clerk' && typeof window.syncStarsFromClerk === 'function') {
                        window.syncStarsFromClerk();
                    } else if (typeof window.syncStarsFromSupabase === 'function') {
                        window.syncStarsFromSupabase();
                    }
                }
            }
        ...
        // Keep the UI in sync when Supabase logs a user in/out (region-aware auth gate).
        if (window.AppAuth) {
