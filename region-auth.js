...
(function () {
    ...
    const AppAuth = {
        // Region — used only to decide whether to show the "restricted in your
        // region" warning next to the Clerk option. It no longer gates whether
        // VK ID counts as logged in: VK ID is available and fully functional
        ...
        // Unified user getters — provider-agnostic. VK ID takes priority when
        // both happen to be present (shouldn't normally happen), otherwise
        // whichever provider actually has a logged-in user wins.
        getUser: () => {
            if (vkAuthState.user) return vkAuthState.user;
            if (window.Clerk?.user) return window.Clerk.user;
            return null;
        },
        getUserId: () => {
            if (vkAuthState.user) return vkAuthState.user.id;
            if (window.Clerk?.user) return window.Clerk.user.id;
            return null;
        },
        getUserName: () => {
            if (vkAuthState.user) return vkAuthState.user.name;
            if (window.Clerk?.user) return window.Clerk.user.fullName || window.Clerk.user.username || 'User';
            return 'Guest';
        },
        getUserEmail: () => {
            if (vkAuthState.user) return vkAuthState.user.email || 'not set';
            if (window.Clerk?.user) return window.Clerk.user.primaryEmailAddress?.emailAddress || 'email@gmail.com';
            return 'not set';
        },
        getProvider: () => {
            if (vkAuthState.user) return 'vk';
